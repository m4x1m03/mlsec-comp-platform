...
def require_admin_origin(request: Request, *, require_present: bool = True) -> None:
    """Ensure Origin/Referer points to localhost or allowed hosts (and is present if required)."""
    settings = get_settings()
    ...
    if require_present and not origin and not referer:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin actions require a valid browser origin",
        )
    ...
    if origin:
        ...
        if not _is_loopback_host(origin_host) and not _is_in_allowed_hosts(
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Admin actions require authorized origin",
            )
